            ...
            if True:
                def enlarge_boxes(boxes):
                    for box in boxes:
                        ...
                        min_growth = 0
                        box_growth_x = max(float((box[2] - box[0]) / 8), min_growth)
                        box_growth_y = max(float((box[3] - box[1]) / 8), min_growth)
                        box[0] = max(0, box[0] - box_growth_x)
                        box[1] = max(0, box[1] - box_growth_y)
                        box[2] = min(w-1, box[2] + box_growth_x)
                        box[3] = min(h-1, box[3] + box_growth_y)
                        
                    return boxes
                boxes = enlarge_boxes(boxes)
