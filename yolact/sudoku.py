...
import puzzle.detector_solver as detector_solver
...
def solve_frame(img, masks, classes, scores, boxes):
    ...
    for i in range(num_detections):
        ...
        threshold score
        grow mask
        masked_image = image x mask
        temp_image = masked_image at bounding box
        solve temp image
        copy back temp image into img_numpy
        """
        ...
        # dilate mask:
        # todo: kernel size and number of iterations depending on mask size
        kernel = np.ones((5,5), np.uint8)
        mask = cv2.erode(mask, kernel, iterations=10) 
        mask = cv2.dilate(mask, kernel, iterations=30) 

        # todo: fill mask's convex hull:

        masked_img = img.copy()
        masked_img[mask[...,None] == [0, 0, 0]] = 1
        # plot_img(masked_img, "masked_img")

        # todo: box_growth depending on box size
        min_growth = 10
        box_growth_x = max(int((box[2] - box[0]) / 10), min_growth)
        box_growth_y = max(int((box[3] - box[1]) / 10), min_growth)
        x0 = max(0, box[0] - box_growth_x)
        y0 = max(0, box[1] - box_growth_y)
        x1 = min(img.shape[1]-1, box[2] + box_growth_x)
        y1 = min(img.shape[0]-1, box[3] + box_growth_y)

        cropped_img = masked_img[y0:y1,x0:x1,:].copy()
        # plot_img(cropped_img, "cropped_img_{}".format(i))

        # cv2.imwrite("./sudoku_cropped_new/image_{}.jpg".format(global_counter), (cropped_img * 255).astype(np.uint8))
        # cv2.imwrite("./sudoku_bb/image_{}.jpg".format(global_counter), (img[y0:y1,x0:x1,:].copy() * 255).astype(np.uint8))
        # global_counter += 1
        # plt.show()


        # check if the image is large enough (num pixels width/height)
        # TODO: check if the image is even sharp enough
        min_size = 300
        ...
        try:
            ...
            # solution, corners, rotation = detector_solver.detect_and_solve(cropped_img)
            cropped_solution = detector_solver.solver((cropped_img * 255).astype(np.uint8))
            if cropped_solution is not None:

                # backproject
                # img[y0:y1,x0:x1,:] = np.swapaxes(cropped_solution, 0, 1) / 255
                full_solution = np.zeros_like(img)
                full_solution[y0:y1,x0:x1,:] = (cropped_solution / 255)
                img[mask[...,None] != [0, 0, 0]] = full_solution[mask[...,None] != [0, 0, 0]]
                # img[mask[...,None] == [0, 0, 0]] =
                pass
        except:
            print("something went wrong in the solver")
    
    return img #, closest_index


def track_frame(img, masks, classes, scores, boxes, solve=False):
    ...
    for i in range(num_detections):
        ...
        if tracking == True:
            ...
            age_threshold = 5
            local_tracker = []
            for (tmp_center, tmp_solution, tmp_focus, age) in tracker:
                if age <= age_threshold:
                    local_tracker.append((tmp_center, tmp_solution, tmp_focus, age + 1))
            tracker = local_tracker


            # find closest object (center and shape):
            closest_index = None
            closest_distance = None
            closest_center = None
            distance_threshold = (boxes[i][2] - boxes[i][0]) * 2 # should depend on box size
            for track_idx, (center_i, solution_i, focus_i, age_i) in enumerate( tracker ):
                
                # (center_i, solution_i, focus_i, age_i) = tracker[track_idx]

                center = np.array([boxes[i][0] + boxes[i][2] / 2.0, boxes[i][1] + boxes[i][3] / 2.0])
                distance = np.linalg.norm(center_i - center)

                if closest_index is None:
                    closest_index = track_idx
            ...
            if closest_distance is not None and closest_distance < distance_threshold:# or (closest_center[0] ):
                # if objects are close enough => reset age
                # tracker[closest_index][3] = 0
                tracker[closest_index] = (tmp_center, None, None, 0)
                tracked_idx = closest_index
                skip_solver = True
            else:
                    ...
                    if solution is not None:
                        ...
                        if tracking == True:
                            ...
                            if tracked_idx is None:
                                tracker.append( (center, solution, None, 0) )
                            else:
                                tracker[closest_index] = (center, solution, None, 0)
                except:
                    pass
                # todo: backproject
...
def sudoku_main(img, masks, classes, scores, boxes, tracking):

    """
        if track is set to True the input is coming from a video and
        we can try to skip the puzzle solving iff it's already done in a previous
        might also be used to overlay solution on a blurry image
    """

    if not tracking:
        return solve_frame(img, masks, classes, scores, boxes)
        # return img
    else:
        # return track_frame(img, masks, classes, scores, boxes)
        return img
...
def parse_args(argv=None):
    ...
    parser.add_argument('--trained_model',
                        default='weights/yolact_sudoku_749_27000.pth', type=str,
    ...
    parser.add_argument('--config', default='yolact_sudoku_config',
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str='', tracking=False):
    """
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        try: # try GPU first
            ...
            masks_gpu = masks[:num_dets_to_consider, :, :, None]
            
            # Prepare the RGB images for each mask given their color (size [num_dets, h, w, 1])
            colors = torch.cat([get_color(j, on_gpu=img_gpu.device.index).view(1, 1, 1, 3) for j in range(num_dets_to_consider)], dim=0)
            masks_color = masks_gpu.repeat(1, 1, 1, 3) * colors * mask_alpha

            # This is 1 everywhere except for 1-mask_alpha where the mask is
            inv_alph_masks = masks_gpu * (-mask_alpha) + 1
        ...
        except: # if out-of-memory occurs try again, but with CPU instead
            print('cuda out of memory: trying again with cpu')
            # After this, mask is of size [num_dets, h, w, 1]
            masks = masks[:num_dets_to_consider, :, :, None].cpu()
            
            # Prepare the RGB images for each mask given their color (size [num_dets, h, w, 1])
            colors = torch.cat([get_color(j, on_gpu=img_gpu.device.index).view(1, 1, 1, 3) for j in range(num_dets_to_consider)], dim=0)
            masks_color = masks.repeat(1, 1, 1, 3).cpu() * colors.cpu() * mask_alpha
            ...
            #    for j in range(num_dets_to_consider):
            #        img_gpu = img_gpu * inv_alph_masks[j] + masks_color[j]
            masks_color_summand = masks_color[0].cpu()
            if num_dets_to_consider > 1:
                inv_alph_cumul = inv_alph_masks[:(num_dets_to_consider-1)].cumprod(dim=0).cpu()
                masks_color_cumul = masks_color[1:].cpu() * inv_alph_cumul
                masks_color_summand += masks_color_cumul.sum(dim=0)

            img_gpu = img_gpu * inv_alph_masks.prod(dim=0).cuda() + masks_color_summand.cuda()
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    def prep_frame(inp, fps_str):
        with torch.no_grad():
            frame, preds = inp
            return prep_display(preds, frame, None, None, undo_transform=False, class_color=True, fps_str=fps_str, tracking=True)
